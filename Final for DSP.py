...
def HighPassFilter():
    ...
    b = scipy.signal.firwin(101, cutoff=1000, fs=sr, pass_zero=False)     #the number after cutoff determines cutoff freq #changeable
    x = scipy.signal.lfilter(b, [1.0], x)
    wavfile.write('HPFed.wav', sr, x.astype(np.int16))

def LowPassFilter():
    sr, x = wavfile.read('Water-Dripping-A4-www.fesliyanstudios.com.wav')  # 16-bit mono 44.1 khz
    b = scipy.signal.firwin(101, .9)  # the 2nd number determines the low pass freq (choose between 0 and 1) # Changeable
    x = scipy.signal.lfilter(b, [1.0], x)
    wavfile.write('LPFed.wav', sr, x.astype(np.int16))
